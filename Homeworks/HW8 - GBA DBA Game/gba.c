...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int r = 0; r < height; r++) {
    DMA[3].src = &color;
    DMA[3].dst = &videoBuffer[OFFSET(col + r, row, 240)];
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for(int r = 0; r < height; r++) {
      ...
      DMA[3].dst = videoBuffer + OFFSET(col + r, row,WIDTH);               
...
//this is for updating the background for things like the score text
void drawBackDMA(int row, int col, int width, int height, const u16 *image) {
  for(int r = 0; r < height; r++) {
      DMA[3].src = image + OFFSET(col + r, row, WIDTH);                
      DMA[3].dst = videoBuffer + OFFSET(col + r, row, WIDTH);               
      DMA[3].cnt = width | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
  }
}
